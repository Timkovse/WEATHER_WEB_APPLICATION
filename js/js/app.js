(() => {
  ...
  function updateRadar() {
    ...
    const configuredSources = arso[key];
    const sources = Array.isArray(configuredSources) ? configuredSources : [configuredSources];
    ...
    const finishWithError = () => {
      ...
      loader.hidden = false;
      loader.textContent = "ARSO slike trenutno ni mogoče prikazati. Uporabi povezavo »Odpri pri ARSO«.";
    };
    const trySource = () => {
      window.clearTimeout(loadTimer);
      if (sourceIndex >= sources.length || !sources[sourceIndex]) {
        finishWithError();
        return;
      }
      ...
      loadTimer = window.setTimeout(trySource, 12000);
      const separator = sourceUrl.includes("?") ? "&" : "?";
      image.src = `${sourceUrl}${separator}v=${Date.now()}`;
    };
    trySource();
